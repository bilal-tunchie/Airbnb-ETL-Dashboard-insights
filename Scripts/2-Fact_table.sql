...
---- Check for unwanted spaces (city)
...
---- Check for invalid and reaarange address order: district, city, country (address_)
SELECT DISTINCT
	city,
	address_
FROM airbnb.Fact_table
...
SELECT 
	...
	CASE
		WHEN city = 'Az Zilfi' THEN city
		ELSE 'Riyadh'
	END AS city,
	CASE
		WHEN address_ LIKE '_ir%' or address_ LIKE '_z%' THEN  address_
		WHEN address_ LIKE N'%العمارية%' THEN  'Al Amaria, Riyadh Province, Saudi Arabia'
		WHEN address_ LIKE '%sul%' THEN  'Sulaymaniyah, Riyadh Province, Saudi Arabia'
		WHEN address_ LIKE 'Riyadh%' AND address_ != 'Riyadh, Riyadh Province, Saudi Arabia'
		THEN  CONCAT( SUBSTRING(address_, 9, LEN(address_) - 21), ' Riyadh Province, Saudi Arabia' )
		ELSE 'Riyadh, Riyadh Province, Saudi Arabia'
	END AS address_,
